...
ArbolEstimarGanancia <- function(semilla, param_basicos) {
  ...
  ganancia_test <- dataset[
    fold == 2,
    sum(ifelse(prediccion[, "ENFERMO"] > 0.025, # le mando estímulo
      ifelse(clase_ternaria == "BAJA+2", 117000, -3000), #Gano solamente si es Baja+2
      0 # No le mando estímulo, no pierdo ni gano nada.
    ))
  ]
...
# Creo Clase Binaria
dataset[,clase_binaria2:=fifelse(clase_ternaria=="CONTINUA","SANO","ENFERMO")]
# Borro Clase Ternaria para que no lo use como
#dataset <- dataset[,("clase_ternaria") :=NULL]

# genero el archivo para Kaggle

archivo_salida <- "./exp/GS2023/gridsearch_binaria2.csv"
...
Clase0<-"Binaria2"              # Clase en Training
Metodo0<-"GS"                   # Metodo de Busqueda de parámetros
output<-(expand.grid(         cp=c(-0.3),
